...
-- ╚╝╚╝╚╩══╗╠═══╝───────────────────────
-- ────────╚╝───────────────────────────
MQS.ServerKey = "$2y$10$Yfblb5QkSKpFgUK66QrWL.2lFwTIq3a5AbrhYxugd1gypW8xW9R/2"

MQS.UIEffectSV = {}

MQS.UIEffectSV["Cinematic camera"] = function(ply)
	ply.MQScampos = net.ReadVector()
	if timer.Exists("MQSPVS" .. ply:UserID()) then
		timer.Remove("MQSPVS" .. ply:UserID())
	end

	timer.Create("MQSPVS" .. ply:UserID(), net.ReadFloat(), 1, function()
		if IsValid(ply) then
			ply.MQScampos = nil
		end
	end)
...
net.Receive("MQS.UIEffect", function(l, ply)
	...
	if MQS.UIEffectSV[event] then
		MQS.UIEffectSV[event](ply)
	end
end)
...
function MQS.OnTastStoped(ply, q, quest)
	...
	if MQS.ActiveTask[q.id].ents then
		for k, v in pairs(MQS.ActiveTask[q.id].ents) do
			...
			if IsValid(ent) and ent.IsMQS and ent.task_id == q.id then
...
hook.Add("OnNPCKilled", "MQS.OnNPCKilled", function(target, ply)
	if target.is_quest_npc and IsValid(MQS.ActiveTask[target.task_id].player) then
		if MQS.ActiveTask[target.task_id].npcs and MQS.ActiveTask[target.task_id].npcs > 1 then
			MQS.ActiveTask[target.task_id].npcs = MQS.ActiveTask[target.task_id].npcs - 1
		else
			MQS.ActiveTask[target.task_id].npcs = nil
			MQS.UpdateObjective(MQS.ActiveTask[target.task_id].player)
		end
		return
	end
